...
현재 DB 파일은 인증 vendor의 `auth` schema와 `business`의 workspace/member 및 BE-04 계정·초대·설정 테이블을 만든다. 운영 DB에는 아직 적용하지 않았다.
...
BE-02 auth migration journal과 BE-03/04 business migration journal은 각각 `drizzle`, `drizzle_business` schema에 분리된다. Drizzle snapshot과 SQL 파일을 함께 관리한다. BE-03/04 SQL의 수동 FK·소유권·RLS·grant 절은 생성된 DDL 뒤에 의도적으로 추가했으므로 후속 `drizzle-kit generate` 출력과 diff를 검토한다. BE-04의 사용자/초대 테이블은 기존 auth 사용자 row를 수정하지 않으며, `auth.user` FK와 개인 workspace FK를 새로 건다. 기존 데이터의 중복·참조 충돌은 적용 전 별도 inventory로 확인한다.
