# IEUM 프로젝트 상태 · BE-12 IMPLEMENTED, CORE-16 BLOCKED

- 검증 기준: BE-12 구현 `071f752`의 로컬 검증(2026-09-24 KST). 직전 원격 Linux 검증은 BE-06 GitHub Actions run `35908300864` 전체 성공
- 실행 정본: [계획 1.1](../plan/README.md), [backlog](../plan/backlog.json)
- 완료 범위: **BASE-01–03, CORE-01–15, BE-01–10 VERIFIED**. 원본·근거·시점 snapshot, lexical/semantic/hybrid 관찰 검색과 합성 평가, 근거 묶음, 추출 후보, 구조 진단·변경안 미리보기, 문서 계획·claim 검증 계약, Nest/Fastify 조립, 실제 DB 인증·업무 workspace/RLS·계정/초대/세션/복구·명령/감사/outbox relay·원문/unit revision·다중 Context 소속과 관계·독립 Task 수명·결과 원본·일정 시간대/DST까지 확인했다.
- 진행 중: **BE-12 snapshot builder·판단 job은 로컬 구현·검증을 마쳤고 Linux CI 확인 전 IMPLEMENTED**다. 통과 후 BE-13으로 진행한다. BE-11은 FE-01→FE-02 선행 검증을 기다린다. **CORE-16은 FE-14·FE-16과 허용된 실제 기록/holdout 부재로 BLOCKED**다.
...
| 75개 작업 카드와 렌더/정합성 검사 | BASE-01–03·CORE-01–15·BE-01–10 VERIFIED, BE-12 IMPLEMENTED, CORE-16 BLOCKED | BE-12 CI·FE-01 |
...
| Nest API·인증·DB·worker·업무 기능 | BE-01–10이 Linux CI 통과. BE-12 판단 job API는 로컬 구현·검증 완료, CI 대기 | BE-12 CI·BE-11/13–26 |
...
BE-12의 권한 범위 snapshot·비동기 판단·재생과 로컬 PostgreSQL 반례는 [BE-12 증거](../evidence/be-12.md)에 있다. Linux CI 확인 전이며 실제 추천 품질은 검증하지 않았다.
