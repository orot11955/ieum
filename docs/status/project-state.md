# IEUM 프로젝트 상태 · BE-03 IMPLEMENTED, CORE-16 BLOCKED

- 검증 기준: main `bd382d3` (2026-09-24 KST, BE-02 GitHub Actions run `35883533850` 성공); BE-03은 로컬 검증 후 Linux CI 대기
- 실행 정본: [계획 1.1](../plan/README.md), [backlog](../plan/backlog.json)
- 완료 범위: **BASE-01–03, CORE-01–15, BE-01–02**. 원본·근거·시점 snapshot, lexical/semantic/hybrid 관찰 검색과 합성 평가, 근거 묶음, 추출 후보, 구조 진단·변경안 미리보기, 문서 계획·claim 검증 계약, Nest/Fastify 조립과 실제 DB 인증 adapter 호환성까지 VERIFIED다.
- 진행 중: **BE-03 업무 schema/RLS는 격리 PostgreSQL에서 5개 반례를 통과해 IMPLEMENTED**다. Linux CI 확인 후 VERIFIED로 올리고 BE-04에 착수한다. **CORE-16은 FE-14·FE-16과 허용된 실제 기록/holdout 부재로 BLOCKED**다.
...
| 75개 작업 카드와 렌더/정합성 검사 | BASE-01–03·CORE-01–15·BE-01–02 VERIFIED, BE-03 IMPLEMENTED, CORE-16 BLOCKED | BE-03 CI·FE-01 |
...
| Nest API·인증·DB·worker·업무 기능 | BE-01 최소 API·BE-02 auth adapter는 Linux CI 통과. BE-03 업무 DB/RLS는 로컬 실제 PostgreSQL 통과, CI 대기. 사용자 membership·업무 API·job 미구현 | BE-03 CI, BE-04–26 |
...
BE-02의 PostgreSQL·CI 시험은 [BE-02 증거](../evidence/be-02.md)와 [auth spike 보고서](../evidence/auth-spike-report.md)에 있다. BE-03의 로컬 PostgreSQL·role/RLS 반례와 남은 CI gate는 [BE-03 증거](../evidence/be-03.md)에 있다.
