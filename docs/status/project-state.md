...
- 완료 범위: **BASE-01의 저장소/문서 준비**
- 다음 구현: **BASE-02 → BASE-03 → CORE-01–08**
...
| 공통 React UI·theme-init·theme unit test | 보존한 소스 표본; runtime/브라우저 미검증 | FE-01, BASE-02 |
| 75개 작업 카드와 렌더/정합성 검사 | 채택, BASE-01만 완료 | BASE-02부터 |
| pnpm workspace·TS·Vitest·제품 build | 미구현/재구성 대기 | BASE-02 |
...
삭제된 제품 경로를 실행하는 npm scripts·의존성/lock·root TS/Vitest/Vite 설정과 깨진 web index를 걷어냈다. 기존 다크 theme-init와 공통 UI/test는 재사용 가치가 있어 보존했다. 옛 제품 패치를 자동 적용하거나 패키지를 자동 고정하는 workflow는 제거하고 지속 CI는 읽기 전용 검사로 제한한다.
...
실제 실행 결과는 [BASE-01 증거](../evidence/base-01.md)와 GitHub Actions의 준비 검사를 따른다. plan/design 검사 통과를 제품 typecheck/build/DB/E2E 통과로 해석하지 않는다. 브랜치 보호 설정을 변경하지 않았으므로 workflow 존재만으로 merge/push 차단이 설정됐다고 주장하지 않는다.

사용자가 구현 시작을 지시하면 [실행 지시문](../plan/07-codex-execution-playbook.md)을 적용한다. 원격 main과 로컬 변경을 먼저 확인하고 현재 검사부터 실행한다. 운영 DB가 있는지 알 수 없으므로 새 격리된 개발 DB 외에는 연결하지 않는다.
