# IEUM 프로젝트 상태 · BE-17 IMPLEMENTED, BE-01–16·FE-02 VERIFIED, CORE-16 BLOCKED

- 검증 기준: BE-15 Linux GitHub Actions run `35921525076`, FE-01 run `36090745845`, FE-02 run `36092642869`, BE-11 run `36094399398`, BE-16 run `36096572669` 네 job 성공(2026-09-25 KST)
- 실행 정본: [계획 1.1](../plan/README.md), [backlog](../plan/backlog.json)
- 완료 범위: **BASE-01–03, CORE-01–15, BE-01–16, FE-01–02 VERIFIED**. 원본·근거·시점 snapshot, lexical/semantic/hybrid 관찰 검색과 합성 평가, 근거 묶음, 추출 후보, 구조 진단·변경안 미리보기/승인·역변경, 문서 계획·claim 검증 계약, Nest/Fastify 조립, 실제 DB 인증·업무 workspace/RLS·계정/초대/세션/복구·명령/감사/outbox relay·원문/unit revision·다중 Context 소속과 관계·독립 Task 수명·결과 원본·일정 시간대/DST·문서 draft/revision·위키 링크·출처 묶음/작업실·제품 observe 판단 job, Paper/Dark 공통 UI와 합성 편집기·폼 spike까지 확인했다.
- 진행 중: BE-17 선택적 모델 정제 adapter·비용·실패 격리의 로컬 합성 검증 통과, Linux CI 대기. **CORE-16은 FE-14·FE-16과 허용된 실제 기록/holdout 부재로 BLOCKED**다.
...
| 75개 작업 카드와 렌더/정합성 검사 | BASE-01–03·CORE-01–15·BE-01–16·FE-01–02 VERIFIED, BE-17 IMPLEMENTED, CORE-16 BLOCKED | BE-17 CI |
...
| Nest API·인증·DB·worker·업무 기능 | BE-01–16 Linux CI 통과. BE-17은 로컬 PostgreSQL·합성 provider 경계 통과, 실제 provider와 Linux CI 미검증. 판단 job은 observe 모드 | BE-17 CI, BE-18–26 |
...
BE-11의 공통 문서 draft/revision·위키 링크, 출처 검증, 충돌·복원 반례와 로컬/Linux PostgreSQL/HTTP 검증은 [BE-11 증거](../evidence/be-11.md)에 있다. 실제 위키 화면의 역순 ACK 처리는 FE-09·FE-10·QA-04에서 검증한다.

BE-06의 pg-boss relay·worker 재시작·재시도·역할 격리와 로컬/Linux PostgreSQL 반례는 [BE-06 증거](../evidence/be-06.md)에 있다.

BE-12의 권한 범위 snapshot·비동기 판단·재생과 로컬/Linux PostgreSQL 반례는 [BE-12 증거](../evidence/be-12.md)에 있다. 실제 추천 품질은 검증하지 않았다.

BE-13의 제안·노출·피드백·승인 transaction과 로컬/Linux PostgreSQL/HTTP 반례는 [BE-13 증거](../evidence/be-13.md)에 있다. 프론트 승인 화면은 아직 구현하지 않았다.

BE-14의 로컬 파서·원문 출처 검증·승인 transaction과 로컬/Linux PostgreSQL/HTTP 반례는 [BE-14 증거](../evidence/be-14.md)에 있다. 일반 문장 추출 모델과 프론트 승인 화면은 아직 구현하지 않았다.
