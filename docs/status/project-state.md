# IEUM 프로젝트 상태 · BE-07 IMPLEMENTED, CORE-16 BLOCKED

- 검증 기준: main `6208e2c` (2026-09-24 KST, BE-05 GitHub Actions run `35894646188` 전체 성공)
- 실행 정본: [계획 1.1](../plan/README.md), [backlog](../plan/backlog.json)
- 완료 범위: **BASE-01–03, CORE-01–15, BE-01–05 VERIFIED**. 원본·근거·시점 snapshot, lexical/semantic/hybrid 관찰 검색과 합성 평가, 근거 묶음, 추출 후보, 구조 진단·변경안 미리보기, 문서 계획·claim 검증 계약, Nest/Fastify 조립, 실제 DB 인증·업무 workspace/RLS·계정/초대/세션/복구·명령/감사/outbox까지 확인했다.
- 진행 중: **BE-07 원문·unit revision IMPLEMENTED**, 로컬 PostgreSQL·HTTP 검증 통과, Linux CI 확인 대기. 다음 서버 카드는 BE-08이다. BE-06은 계획의 P4 판단 연결 단계에서 실제 비동기 작업과 함께 검증한다. **CORE-16은 FE-14·FE-16과 허용된 실제 기록/holdout 부재로 BLOCKED**다.
...
| 75개 작업 카드와 렌더/정합성 검사 | BASE-01–03·CORE-01–15·BE-01–05 VERIFIED, BE-07 IMPLEMENTED, CORE-16 BLOCKED | BE-07 CI·BE-08·FE-01 |
...
| Nest API·인증·DB·worker·업무 기능 | BE-01–05가 Linux CI 통과. BE-07 원문·unit revision은 로컬 DB/HTTP 통과. job API 미구현 | BE-07 CI·BE-08–26 |
...
BE-07의 원문·unit revision, UTF-16 분할, 출처 중복, 개인 공간 격리와 로컬 PostgreSQL/HTTP 반례는 [BE-07 증거](../evidence/be-07.md)에 있다. Linux CI 결과는 해당 증거에 별도로 기록한다.
