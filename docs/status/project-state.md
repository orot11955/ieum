# IEUM 프로젝트 상태 · BE-14 IMPLEMENTED, CORE-16 BLOCKED

- 검증 기준: BE-13 구현 `3918dd0`와 Linux GitHub Actions run `35915443644` 네 job 성공, BE-14 구현 `9b4d987`의 로컬 PostgreSQL·API 검증(2026-09-24 KST)
- 실행 정본: [계획 1.1](../plan/README.md), [backlog](../plan/backlog.json)
- 완료 범위: **BASE-01–03, CORE-01–15, BE-01–10, BE-12–13 VERIFIED**. 원본·근거·시점 snapshot, lexical/semantic/hybrid 관찰 검색과 합성 평가, 근거 묶음, 추출 후보, 구조 진단·변경안 미리보기, 문서 계획·claim 검증 계약, Nest/Fastify 조립, 실제 DB 인증·업무 workspace/RLS·계정/초대/세션/복구·명령/감사/outbox relay·원문/unit revision·다중 Context 소속과 관계·독립 Task 수명·결과 원본·일정 시간대/DST·제품 observe 판단 job까지 확인했다.
- 진행 중: **BE-14 추출 후보 승인·업무 항목 생성은 IMPLEMENTED**, Linux CI를 확인 중이다. 검증 후 다음 독립 서버 카드는 BE-15다. BE-11은 FE-01→FE-02 선행 검증을 기다린다. **CORE-16은 FE-14·FE-16과 허용된 실제 기록/holdout 부재로 BLOCKED**다.
...
| 75개 작업 카드와 렌더/정합성 검사 | BASE-01–03·CORE-01–15·BE-01–10·BE-12–13 VERIFIED, BE-14 IMPLEMENTED, CORE-16 BLOCKED | BE-14 CI·FE-01 |
...
| Nest API·인증·DB·worker·업무 기능 | BE-01–10·BE-12–13 Linux CI 통과, BE-14 로컬 DB/API 통과. 판단 job은 observe 모드 | BE-11·BE-14 CI·BE-15–26 |
...
BE-14의 로컬 파서·원문 출처 검증·승인 transaction과 로컬 PostgreSQL/HTTP 반례는 [BE-14 증거](../evidence/be-14.md)에 있다. 일반 문장 추출 모델과 프론트 승인 화면은 아직 구현하지 않았다.
