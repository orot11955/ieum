...
계획 1.1은 main에 채택됐다. BASE-01은 소스/문서 준비이며 제품 구현 허가가 아니다. **명시적인 구현 지시가 오기 전 BASE-02·Core·API·Web 개발을 시작하지 않는다.** 운영 DB와 배포 환경은 미접근이다.

문서 우선순위: 사용자 최신 지시 → AGENTS.md → ADR 0012 → docs/plan/backlog.json 및 00–08 → 보존된 제품/도메인/화면 계약. 과거 M/S/I 번호·다크 후속·별도 브랜치 지시는 현재 실행 규칙이 아니다.

## 사용자가 “구현 시작”이라고 지시했을 때
...
다음 카드는 BASE-02다. BASE-02 → BASE-03 → CORE-01–08을 우선 구현하라.
...
현재는 README의 Node 기반 plan/design 검사만 실행 가능하다. `pnpm install --frozen-lockfile`, `pnpm lint`, `pnpm typecheck`, `pnpm test:unit`, `pnpm test:architecture`, `pnpm test:contracts`, `pnpm test:integration`, `pnpm test:e2e`, `pnpm build`, `pnpm lab replay/inspect/compare/evidence-pack`은 해당 카드에서 만들어 검증할 **미래 계약**이다. 없는 테스트를 `--passWithNoTests`로 성공 처리하지 않는다.
