...
| 편집기 | `packages/contracts/src/editor.ts` | JSON envelope에 `schemaVersion: 1`을 둔다. FE-02가 block/node schema를 검증한다. 이 버전과 Capture 원문 UTF-16 offset은 다른 계약이다. |
...
- FE-02는 editor node/block ID 유지·한글 IME·serialize/deserialize와 schemaVersion migration을 고정한다. BASE-03 envelope만으로 본문 의미를 검증했다고 표시하지 않는다.
