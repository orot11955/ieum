...
상태: **구현 계획**. 이 문서는 새 작업 카드나 구현 허가가 아니다. 75개 카드의 범위·선행·상태·완료 기준은 [backlog.json](backlog.json)이 정본이다. 아래 순서는 그 선행 그래프를 사용자 기능 단위로 풀어 쓴 실행안이다. 현재 공통 UI 소스와 core/lab 최소 빌드·계약 예제·원본 근거 모델은 있지만 판단 Core 로직/API/업무 웹 기능은 미구현이다. BASE-01–03과 CORE-01은 VERIFIED다.
