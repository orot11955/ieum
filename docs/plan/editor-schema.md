...
- 영속 편집 JSON의 문자열에는 NUL과 짝이 없는 UTF-16 surrogate를 허용하지 않는다. `unit` 출처의 `sourceHash`는 해당 불변 unit revision의 `content_text`를 UTF-8로 SHA-256 해시한 값이고, 선택적 span은 그 텍스트의 UTF-16 범위다. `document_revision` 출처의 `sourceHash`는 스키마 검증 후 정규화한 editor JSON의 UTF-8 SHA-256 값이고 `originKey`는 `document:<id>`다. 이 출처의 선택적 span은 불변 revision을 블록별 줄바꿈으로 연결하고 inline text·hardBreak의 줄바꿈·sourceReference의 화면 label을 순서대로 이어 만든 **파생 평문**의 UTF-16 범위다. BE-16의 `external_excerpt` 출처는 같은 workspace의 불변 발췌 revision·본문 SHA-256·`external_excerpt:<id>` originKey·선택적 UTF-16 span을 확인하며 삭제 상태면 새 참조 저장을 거부한다. 이 범위는 영구 block/claim ID를 대신하지 않으며 문장 의미의 지지를 보증하지 않는다.
...
## BE-16 자료 묶음과 작업실 mapping

- `EvidencePack`은 문서 ID에 속하고, 각 pack revision은 선택 당시의 source kind/ID/revision/originKey/본문 hash/선택 범위·텍스트/제목·외부 메타데이터를 고정한다. 조회 시 현재 출처와 비교한 `fresh`·`stale`·`unresolved`는 동적으로 계산하며 옛 manifest를 갱신하지 않는다. `originKey`가 같은 재인용은 독립 출처 수를 늘리지 않는다. 외부 URL은 출처 메타데이터이며 서버가 fetch하지 않는다.
- 작업실 revision은 정확한 pack revision·현재 draft version·목적·독자·체크리스트 outline·충돌·claim mapping·출처 manifest snapshot을 보존한다. `claim.textHash`는 해당 block의 inline text, `hardBreak`의 `\n`, `sourceReference`의 화면 label을 순서대로 합친 **블록 평문**을 UTF-8로 SHA-256 해시한 값이다. `claim.blockHash`는 `canonicalEditorBlock(block)`의 UTF-8 SHA-256 값이며 정렬된 JSON key와 전체 `sourceReference.attrs.ref`를 포함한다. 두 hash 중 하나가 달라지면 `needs_remap`으로 표시한다. 화면 label과 평문이 같아도 출처 ID/revision만 바뀌면 재검토한다.
- 조회의 `claimStates`는 블록이 없거나 평문 hash가 바뀌면 `needs_remap`, 출처가 삭제·비가시화되면 `source_unresolved`, revision/hash가 바뀌면 `source_stale`로 표시한다. 둘 이상 해당하면 remap을 우선 표시하므로 UI는 별도의 `sourceStates`도 함께 확인한다. `readiness`는 누락 자료와 수동 검토 항목을 알려주는 도우미이며 출처의 사실성·발행 승인이 아니다.
