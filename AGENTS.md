...
- BASE-01–03과 CORE-01–15는 로컬 검증과 Linux CI까지 VERIFIED다. CORE-07/09/10의 공개 품질 수치는 합성 자료에 한정되며 실제 사용자 품질이나 활성 추천 정책을 입증하지 않는다. CORE-16은 FE-14·FE-16과 허용된 실제 자료·holdout이 없어 BLOCKED다. 문서/정리 요청만으로 제품 기능이나 DB/배포를 변경하지 않는다.
- 다음 카드 전 git status·원격 main·현재 prep 검사와 선행 카드 증거를 확인한다. 현재 준비된 제품 시작 카드는 BE-01·FE-01과 QA-02다. CORE-16 재개 조건은 `docs/evidence/core-16.md`를 따른다. main이 변경됐다면 diff를 재검토하고 사용자 변경을 보존한다.
